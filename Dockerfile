# offhook-agent — production voice-agent worker image.
#
...
#   • or extend:    FROM ghcr.io/you/offhook-agent  +  COPY agent.yaml knowledge/ ./
#
...
ENV OFFHOOK_AGENT_CONFIG=/app/agent.yaml
...
# `offhook-agent start` hands off to the LiveKit worker (registers, pools, dispatches
# each inbound call to the entry hook, drains on SIGTERM).
CMD ["node", "bin/offhook-agent.js", "start"]
