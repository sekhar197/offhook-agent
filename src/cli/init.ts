/**
 * `offhook-agent init` — guided setup. Pick how far you want to go (a tier), pick your
 * providers, and offhook-agent opens each key page for you and grabs the key off your
...
export async function initCommand(targetDir: string): Promise<void> {
  intro('offhook-agent — set up your voice agent');
  ...
  const agentYaml = `# Generated by \`offhook-agent init\` — edit freely. Docs: https://github.com/sekhar197/offhook-agent
agent:
  id: ${id}
...
`;
  ...
  const next: Record<Tier, string> = {
    local: '`ollama serve` + `docker compose -f docker-compose.selfhost.yml up`, then `offhook-agent chat`',
    chat: '`offhook-agent doctor`, then `offhook-agent chat`',
    browser: '`offhook-agent doctor`, then `offhook-agent dev` — talk in your browser',
    phone: '`offhook-agent doctor`, then `offhook-agent phone provision` → `offhook-agent phone connect` → `offhook-agent start`',
  };
